...
export function ThemeToggle({
  ...
  React.useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Toggle theme on pressing 'd' or 'D' when not typing in an input/textarea
  ...
  return (
    <Button
      ...
      title="Toggle Light/Dark Theme (Press 'D')"
    >
      ...
      {showLabel && (
        <span className="ml-2 font-medium text-xs capitalize">{resolvedTheme} theme</span>
      )}
    </Button>
  );
}
